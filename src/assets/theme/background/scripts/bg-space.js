...
function getStarfield({ numStars = 1200, radius = 50, exclusionRadius = 30 } = {}) {
...
function getBackground() {
  ...
  const geometry = new THREE.SphereGeometry(500, 16, 12); // reduced segments
...
function initialiseBackground(container) {
  ...
  const sunLight = new THREE.PointLight(0xffffee, 1350.0, 999999);
  sunLight.position.set(0, 0, 0);
  sunLight.castShadow = false; // disabled for perf
  scene.add(sunLight);

  const earthGroup = new THREE.Group();
  scene.add(earthGroup);

  const earthDetail = 20; // reduced from 60
  ...
  const marsDetail = 20; // reduced
  const marsGeometry = new THREE.IcosahedronGeometry(1, marsDetail);
  const marsMaterial = new THREE.MeshPhongMaterial({
    map: new THREE.TextureLoader().load(mars_mosaic_1),
  });
  ...
  const stars = getStarfield({ numStars: 1200, radius: 50, exclusionRadius: marsOrbitMaxRadius });
  scene.add(stars);
